...
param(
    ...
    [switch]$BluRayOnly = $false,  # Only auto-rip BluRay discs (skip DVDs)
    [int]$MaxDVDRips = 2,
    [int]$MaxBluRayRips = 1,
    [int]$MaxCDRips = 1,
    [int]$MaxGameRips = 1,
...
function Test-VideoGameDisc {
    ...
    # Get disc size to avoid false positives on DVD movie discs with bonus PC content
    $Volume = Get-Volume -DriveLetter $DriveLetter -ErrorAction SilentlyContinue
    $DiscSizeGB = if ($Volume.Size) { $Volume.Size / 1GB } else { 0 }
    
    # DVD movies are typically < 9GB, BluRay games are > 20GB
    # Only allow generic PC game detection (*.exe) on large discs
    $IsDVDSized = $DiscSizeGB -lt 9
    
    # Check volume label for common game disc patterns
    # Console-specific patterns are allowed on any size disc
    $ConsolePatterns = @(
        # PlayStation (can be on DVD-sized discs for PS2)
        "^PS2",
        "^PS3", 
        "^PS4",
        "^PS5",
    ...
    # Generic patterns that could false-positive on movie DVDs with bonus content
    # Only check these on BluRay-sized discs
    $GenericPCPatterns = @(
        "GAME",
        "INSTALL",
        "SETUP"
    ...
    # Always check console patterns
    foreach ($Pattern in $ConsolePatterns) {
        if ($VolumeLabel -match $Pattern) {
            Write-Log "Console game disc pattern matched: $Pattern in volume label: $VolumeLabel" -Level "SUCCESS"
            return $true
        }
    }
    
    # Only check generic PC patterns on large discs (not DVD-sized)
    if (-not $IsDVDSized) {
        foreach ($Pattern in $GenericPCPatterns) {
            if ($VolumeLabel -match $Pattern) {
                Write-Log "PC game disc pattern matched: $Pattern in volume label: $VolumeLabel (disc size: $([math]::Round($DiscSizeGB, 1)) GB)" -Level "SUCCESS"
                return $true
            }
        }
    }

    # Check for console-specific files (allowed on any size disc)
    try {
        ...
        foreach ($Indicator in $ConsoleIndicators) {
            $Found = Get-ChildItem -Path $DriveRoot -Filter $Indicator -Recurse -ErrorAction SilentlyContinue -Depth 2 | Select-Object -First 1
            if ($Found) {
                Write-Log "Console game indicator found: $Indicator in $DriveRoot" -Level "SUCCESS"
                return $true
            }
        }
        
        # Only check for *.exe on BluRay-sized discs (avoid false positives on DVD bonus content)
        if (-not $IsDVDSized) {
            $Found = Get-ChildItem -Path $DriveRoot -Filter "*.exe" -Recurse -ErrorAction SilentlyContinue -Depth 2 | Select-Object -First 1
            if ($Found) {
                Write-Log "PC game indicator found: *.exe in $DriveRoot (disc size: $([math]::Round($DiscSizeGB, 1)) GB)" -Level "SUCCESS"
                return $true
            }
        } else {
            Write-Log "Skipping *.exe check on DVD-sized disc ($([math]::Round($DiscSizeGB, 1)) GB) - likely movie with bonus content" -Level "INFO"
        }
    } catch {
...
function Get-MakeMKVDriveIndex {
    param([string]$DriveLetter)
    
    # Query MakeMKV for actual drive mapping
    try {
        $InfoOutput = & $MakeMKVPath -r info disc:9999 2>&1 | Out-String
        
        # Parse DRV lines to find our drive letter
        foreach ($Line in ($InfoOutput -split "`n")) {
            if ($Line -match 'DRV:(\d+),\d+,\d+,\d+,"[^"]*","[^"]*","([A-Z]):"') {
                $DiscIndex = [int]$Matches[1]
                $DiscDrive = $Matches[2]
                if ($DiscDrive -eq $DriveLetter) {
                    Write-Log "MakeMKV drive mapping: ${DriveLetter}: = disc:$DiscIndex"
                    return $DiscIndex
                }
            }
        }
    } catch {
        Write-Log "ERROR: Failed to query MakeMKV drive index - $($_.Exception.Message)" -Level "ERROR"
    }
    
    # Fallback to old calculation if query fails
    $FallbackIndex = [byte][char]$DriveLetter - [byte][char]'D'
    Write-Log "WARNING: Using fallback drive index $FallbackIndex for ${DriveLetter}:" -Level "WARNING"
...
function Start-VideoRip {
    ...
    # Get drive index
    $DriveIndex = Get-MakeMKVDriveIndex -DriveLetter $DriveLetter
...
function Test-RipOutputValid {
    param(
        [string]$OutputDir,
        [string]$DiscType,
        [int]$MinSizeMB = 100  # Minimum file size in MB
    )
    
    if (-not (Test-Path $OutputDir)) {
        Write-Log "  VALIDATION FAILED: Output directory does not exist: $OutputDir" -Level "ERROR"
        return $false
    }
    
    # Get all MKV files in output directory
    $OutputFiles = Get-ChildItem -Path $OutputDir -Filter "*.mkv" -ErrorAction SilentlyContinue
    
    if ($OutputFiles.Count -eq 0) {
        Write-Log "  VALIDATION FAILED: No MKV files found in $OutputDir" -Level "ERROR"
        return $false
    }
    ...
    foreach ($File in $OutputFiles) {
        ...
        if ($SizeMB -ge $MinSizeMB) {
            $ValidFiles += $File
            Write-Log "  Valid output: $($File.Name) ($SizeMB MB)" -Level "SUCCESS"
        } else {
            $InvalidFiles += $File
            Write-Log "  INVALID output (too small): $($File.Name) ($SizeMB MB < $MinSizeMB MB minimum)" -Level "ERROR"
        }
    }
    
    if ($ValidFiles.Count -eq 0) {
        Write-Log "  VALIDATION FAILED: All $($OutputFiles.Count) output files are under minimum size ($MinSizeMB MB)" -Level "ERROR"
        # Delete invalid small files
        foreach ($File in $InvalidFiles) {
            try {
                Remove-Item -Path $File.FullName -Force
                Write-Log "  Deleted invalid file: $($File.Name)" -Level "WARNING"
            } catch {
                Write-Log "  Could not delete invalid file: $($File.Name)" -Level "WARNING"
            }
        }
        return $false
    }
    
    Write-Log "  VALIDATION PASSED: $($ValidFiles.Count) valid file(s) in output" -Level "SUCCESS"
    return $true
}
